...
with st.sidebar:
    ...
    # ── Template filter — user pins which trained template to prefer ────
    with st.expander("🎯 Template Preference (iFlow)", expanded=True):
        st.caption(
            "Type part of a template name to prefer it over others. "
            "Leave blank to let the tool pick the best match automatically. "
            "Example: type **Smartapp** to always use Smartapp ONE templates."
        )
        st.text_input(
        ...
        if pref:
            ...
            if matches:
                ...
                st.warning(f"No trained templates match '{pref}' — will use skeleton", icon="⚠️")
        else:
        ...
        for i, q in enumerate(iflow_quick):
            if st.button(q, key=f"qif_{i}", use_container_width=True):
                ...
                if smartapp_prompt_requested(q):
                    # Capture input values NOW (at click time) into pending_cfg
                    # so they survive the st.rerun() safely.
